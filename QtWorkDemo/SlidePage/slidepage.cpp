...
void SlidePage::addPage(QWidget *w)
{
    ...
    label->setPixmap(QPixmap(":/image/icons/indicator1.png"));
...
void SlidePage::onCurrentPageIndexChanged(int index)
{
    for (int i = 0; i < pageIndicator.count(); i++) {
        if (i == index)
            pageIndicator[i]->setPixmap(QPixmap(":/image/icons/indicator2.png"));
        else
            pageIndicator[i]->setPixmap(QPixmap(":/image/icons/indicator1.png"));
    }
}
